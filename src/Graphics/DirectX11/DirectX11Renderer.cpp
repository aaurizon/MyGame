...
HFONT getOrCreateFont(int size, std::vector<DirectX11Renderer::FontEntry>& cache) {
    ...
    if (font) {
        cache.push_back(DirectX11Renderer::FontEntry{size, font});
    }
    return font;
}

void drawOverlayText(HDC dc, const AViewport& viewport, std::vector<DirectX11Renderer::FontEntry>& fontCache) {
    if (!dc) {
        return;
    }
...
bool DirectX11Renderer::initialize(void* nativeWindow, int width, int height) {
...
void DirectX11Renderer::resize(int width, int height) {
    width_ = width;
    height_ = height;
    ensureBackBuffer(width_, height_);
}

void DirectX11Renderer::setWorld(AWorld* world) {
    world_ = world;
}

void DirectX11Renderer::draw(const AViewport& viewport) {
    if (!hwnd_) {
        return;
    }
...
void DirectX11Renderer::releaseBackBuffer() {
    if (backBufferDC_) {
